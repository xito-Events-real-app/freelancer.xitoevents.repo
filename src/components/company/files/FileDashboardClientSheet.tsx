...
import { CheckCircle, Clock, HardDrive, Copy } from "lucide-react";
...
export function FileDashboardClientSheet({ file, open, onOpenChange, onUpdated }: Props) {
  ...
  const handleMarkCopied = async () => {
    ...
    await updateFileRecord(file.id, { confirmed: true, synced_to_sheet: false }, { agencyId: requireAgency() });
    scheduleFilesPush();
  ...
  const handleMarkDoubleBackup = async () => {
    ...
    await updateFileRecord(file.id, { double_backup: true, synced_to_sheet: false }, { agencyId: requireAgency() });
    scheduleFilesPush();
        ...
        <ScrollArea className="h-[calc(100vh-100px)] pr-2 mt-4">
          <div className="space-y-5">
            ...
            <div className="rounded-xl bg-[hsl(220,25%,12%)] p-4 grid grid-cols-3 gap-3 text-center">
              <div><p className="text-xs text-muted-foreground">Total</p><p className="text-lg font-bold">{Number(file.size_gb) || 0} GB</p></div>
              <div><p className="text-xs text-muted-foreground">Photo</p><p className="text-lg font-bold">{photoSize} GB</p></div>
              <div><p className="text-xs text-muted-foreground">Video</p><p className="text-lg font-bold">{videoSize} GB</p></div>
            </div>
            ...
            <div className="rounded-xl bg-[hsl(220,25%,12%)] p-4 space-y-3">
              ...
              <div><p className="text-xs text-muted-foreground">Primary Path</p><p className="text-xs font-mono break-all">{file.final_generated_path || "—"}</p></div>
              <div><p className="text-xs text-muted-foreground">Backup 2</p><p className="text-xs font-mono break-all">{file.backup_2_path || "—"}</p></div>
              <div><p className="text-xs text-muted-foreground">Backup 3</p><p className="text-xs font-mono break-all">{file.backup_3_path || "—"}</p></div>
            </div>
