...
import {
  buildFilePath, StorageDevice, FileRecord, getNextBackupNumber,
  duplicateFileRowForCard, updateFileRecord, deleteFileRecord,
} from "@/lib/files-api";
...
function getNormalizedYearEventFolder(existingFolder: string | null | undefined, eventMonth: string | null | undefined, eventYear: string | null | undefined): string {
  ...
  if (monthNum && eventYear) return `${NEPALI_MONTHS[monthNum] || eventMonth} EVENTS ${eventYear}`;
  return existing;
}
...
export function FilePathBuilderDialog({ open, onOpenChange, fileRecord, devices, onSave, allFiles, onRefresh, initialBackupNumber }: Props) {
  ...
  // "Who Copied First" is restricted to the company's accepted staff only.
  const copierOptions = useMemo(() => {
    const seen = new Set<string>();
    const list: string[] = [];
    for (const raw of staffNames) {
      const n = (raw || "").trim();
      if (!n) continue;
      const key = n.toUpperCase();
      if (seen.has(key)) continue;
      seen.add(key);
      list.push(n.toUpperCase());
    }
    return list;
  }, [staffNames]);
  ...
  useEffect(() => {
    ...
    const defaultYearFolder = getNormalizedYearEventFolder(fileRecord.year_event_folder, fileRecord.event_month, fileRecord.event_year);
    ...
    const initialForms: Record<string, CardFormData> = { [fileRecord.card_label || "1"]: formData };
    for (const card of existingCards) {
      if (card.id !== fileRecord.id) {
        initialForms[card.card_label || "1"] = {
  ...
  const updateCurrentForm = useCallback((updates: Partial<CardFormData>) => {
    setCardForms(prev => ({ ...prev, [activeCard]: { ...prev[activeCard], ...updates } }));
  }, [activeCard]);
  ...
  }, [currentForm, selectedDevice, activeCard]);
  ...
  const handleAddCard = async () => {
    ...
    setCardForms(prev => ({
      ...
      [newKey]: {
        ...
        side: currentForm.side,
        freelancerName: currentForm.freelancerName,
        sizeGb: "", numberOfItems: "",
        formatType: currentForm.formatType,
      },
    }));
    try {
      const newRow = await duplicateFileRowForCard(fileRecord.id, newCardNum, { agencyId: requireAgency() });
  ...
  const handleRemoveCard = async (cardNumber: string) => {
    ...
    if (!cardFile) { toast.error(`Card ${cardNumber} not found`); return; }
    try {
      await deleteFileRecord(cardFile.id, { agencyId: requireAgency() });
      setCardForms(prev => { const next = { ...prev }; delete next[cardNumber]; return next; });
      ...
      setCardCount(newMax);
      if (activeCard === cardNumber) setActiveCard(remainingKeys[0] || "1");
  ...
  const existingCardKeys = useMemo(() => Object.keys(cardForms).sort((a, b) => Number(a) - Number(b)), [cardForms]);
  ...
  const handleRemoveBackup = async () => {
    ...
    try {
      ...
      await updateFileRecord(fileRecord.id, clearUpdates, { agencyId: requireAgency() });
      onOpenChange(false);
      if (onRefresh) await onRefresh();
      scheduleFilesPush();
      toast.success(`Backup ${bn} removed.`);
    } catch (err: any) {
      toast.error("Failed to remove backup: " + (err.message || ""));
    } finally { setRemovingBackup(false); }
  };
  ...
  const handleSave = async () => {
    ...
    if (backupNumber >= 2) {
      ...
      if (!anyFilled) { toast.error("Please set a storage location for at least one card"); return; }
    } else if (keys.length > 1) {
      for (const key of keys) {
        const cf = cardForms[key];
        if (!cf || !cf.storageType || !cf.deviceId) {
          toast.error(`Card ${key} is incomplete`);
          setActiveCard(key);
          return;
        }
      }
    ...
    if (backupNumber >= 2 && fileRecord) {
      ...
      if (backupNumber === 2 && selectedDeviceName && selectedDeviceName === fileRecord.backup_1_device_name) {
        toast.error("2nd backup cannot be on the same device as 1st backup!"); return;
      }
      if (backupNumber === 3 && selectedDeviceName &&
        ...
        toast.error("3rd backup cannot be on the same device as 1st or 2nd backup!"); return;
      }
    }
    ...
    try {
      ...
      if (Object.keys(cardForms).length > 1 && allFiles) {
        for (const key of Object.keys(cardForms)) {
          ...
          await updateFileRecord(cardFileId, { ...cardUpdates, synced_to_sheet: false }, { agencyId: requireAgency() });
        }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        ...
        {existingCardKeys.length > 0 && (
          <div className="flex items-center gap-2 p-2.5 rounded-lg bg-gradient-to-r from-orange-50 to-yellow-50 dark:from-orange-950/30 dark:to-yellow-950/30 border border-orange-300 dark:border-orange-700 flex-wrap">
            ...
            {existingCardKeys.map(key => (
              <div key={key} className="flex items-center gap-0.5">
                <Button variant={activeCard === key ? "default" : "outline"} size="sm" className={cn("h-7 text-xs px-3 font-bold", activeCard === key ? "bg-orange-600 hover:bg-orange-700 text-white" : "border-orange-300 text-orange-700 hover:bg-orange-100 dark:border-orange-700 dark:text-orange-400 dark:hover:bg-orange-950/40")} onClick={() => setActiveCard(key)}>
                  Card {key}
                </Button>
                {key !== "1" && (
                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-500 hover:text-red-700" onClick={() => handleRemoveCard(key)} title={`Remove Card ${key}`}>
            ...
            {cardCount < 4 && (
              <Button variant="ghost" size="sm" className="h-7 text-xs text-orange-700 dark:text-orange-400" onClick={handleAddCard}>
        ...
        <div className={cn(
          ...
          <span className="flex-1 text-center">{backupNumber > 0 ? `Setting ${backupLabel}` : "All 3 Backups Complete ✓"}</span>
          {initialBackupNumber !== undefined && initialBackupNumber >= 1 && initialBackupNumber <= 3 && (
            <Button variant="destructive" size="sm" className="h-7 text-xs ml-2" onClick={handleRemoveBackup} disabled={removingBackup}>
              <Trash2 className="w-3 h-3 mr-1" /> {removingBackup ? "Removing..." : `Remove ${backupLabel}`}
            </Button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-3 p-4 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800">
            <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider">📂 Storage & Path</p>
                        ...
                        <CommandList className="max-h-[320px]">
                          ...
                          <CommandGroup heading={`${filteredDevices.length} devices`}>
                            {[...filteredDevices].sort((a, b) => (a.device_name || "").localeCompare(b.device_name || "")).map((d) => (
                              <CommandItem key={d.id} value={`${d.device_name} ${d.remaining_storage_gb}GB`} onSelect={() => { updateCurrentForm({ deviceId: d.id }); setDevicePopoverOpen(false); }} className="flex items-center justify-between py-2.5 px-3 cursor-pointer">
            ...
            {selectedDevice?.safety_status === "UNSAFE" && (
              <div className="flex items-center gap-2 p-1.5 rounded bg-red-100 dark:bg-red-950/30 text-red-600 text-xs font-bold">
                <AlertTriangle className="w-3.5 h-3.5" /><span>UNSAFE device!</span>
              </div>
            )}
          ...
          <div className="space-y-3 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800">
            <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">💾 Cards & File Info</p>
            ...
            <div className="space-y-2">
              ...
              {existingCardKeys.length > 1 && (
                <div className="flex flex-wrap gap-1">
                  {existingCardKeys.map(key => (
                    <div key={key} className="flex items-center gap-0.5">
                      <Button variant={activeCard === key ? "default" : "outline"} size="sm" className={cn("h-6 text-xs px-2", activeCard === key && "bg-emerald-600 hover:bg-emerald-700 text-white")} onClick={() => setActiveCard(key)}>
                        Card {key}
                      </Button>
                      {key !== "1" && (
                        <Button variant="ghost" size="sm" className="h-5 w-5 p-0 text-red-500 hover:text-red-700" onClick={() => handleRemoveCard(key)} title={`Remove Card ${key}`}>
          ...
          <div className="space-y-3 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800">
            {backupNumber === 1 && (
              <>
                <p className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider">👤 Who Copied First?</p>
                <Select value={whoCopied} onValueChange={setWhoCopied}>
                  <SelectTrigger className="h-8 text-xs">
                    <SelectValue placeholder={copierOptions.length ? "Select staff member..." : "No staff added yet"} />
                  </SelectTrigger>
                  <SelectContent>
                    {copierOptions.length === 0 ? (
                      <div className="px-2 py-1.5 text-xs text-muted-foreground">
                        Add staff in Company → My Freelancers
                      </div>
                    ) : (
                      copierOptions.map(name => (
