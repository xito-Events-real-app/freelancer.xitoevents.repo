...
export function CloudUploadDialog({ open, onOpenChange, fileRecord, devices, onSave }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (!cloudDeviceName) { toast.error("Please select a cloud device"); return; }
    setSaving(true);
    try {
  ...
  const handleRemove = async () => {
    ...
    try {
      await onSave(fileRecord.id, {
        drive_upload: false, drive_upload_path: "", drive_link: "", synced_to_sheet: false,
      });
      onOpenChange(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-row">
          {isEditing && (
            <Button variant="destructive" size="sm" onClick={handleRemove} disabled={saving} className="mr-auto">
              <Trash2 className="w-3.5 h-3.5 mr-1" /> Remove Cloud Backup
            </Button>
          )}
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={handleSave} disabled={saving || !cloudDeviceName} className="bg-purple-600 hover:bg-purple-700 text-white font-bold">
