...
import { AddStorageDeviceDialog } from "./AddStorageDeviceDialog";
...
export function StorageDevicesSection({ deviceTypeFilter, drawerOpen: externalDrawerOpen, onDrawerOpenChange }: Props = {}) {
  ...
  const handleEdit = (device: StorageDevice) => { setEditDevice(device); setDrawerOpen(true); };
  const handleAdd = () => { setEditDevice(null); setDrawerOpen(true); };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredDevices.map((device) => {
            ...
            const usedPct = device.total_storage_gb > 0
              ...
              : 0;
            const remainingPct = device.total_storage_gb > 0 ? device.remaining_storage_gb / device.total_storage_gb : 1;
            const isLowStorage = remainingPct < 0.1;
            const isUnsafe = !isCloudDevice && device.safety_status !== "SAFE";
            ...
            return (
              <Card key={device.id} className={cn("border shadow-sm transition-all hover:shadow-md", hasWarning && "border-red-300 dark:border-red-800")}>
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      <div className={cn("p-2 rounded-lg",
                        isCloudDevice ? "bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {isCloudDevice ? (device.cloud_type || "Cloud") : (
                            <>{device.device_type.replace("_", " ")}{device.pc_drive_letter ? ` (${device.pc_drive_letter}:)` : ""}</>
                          )}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500" onClick={() => {
                        if (confirm(`Delete device "${device.device_name}"?`)) remove(device.id);
                      }}>
                  ...
                  <div className="space-y-1">
                    ...
                    <Progress value={usedPct} className={cn("h-2", isLowStorage ? "[&>div]:bg-red-500" : "[&>div]:bg-blue-500")} />
                    <p className="text-xs text-right text-muted-foreground">{formatStorage(device.total_storage_gb)} total</p>
                  </div>
                    ...
                    {isCloudDevice ? (
                      expiryStatus ? (
                        <span className={cn("text-xs font-medium flex items-center gap-0.5", expiryStatus.color)}>
                          <CalendarClock className="w-3 h-3" />{expiryStatus.label}
                        </span>
                      ...
                      <>
                        <div className="flex items-center gap-2">
                          <span className={cn("text-xs font-medium", healthColor(device.health_percent))}>
                            <Shield className="w-3 h-3 inline mr-0.5" />{device.health_percent}% Health
                          </span>
                          {safetyBadge(device.safety_status)}
                        </div>
                        <div className="flex gap-0.5">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <div key={i} className={cn("w-1.5 h-3 rounded-sm", i < device.speed_rating ? "bg-blue-500" : "bg-muted")} />
                          ))}
                        </div>
                      </>
                    )}
                  ...
                  {hasWarning && (
                    <div className="flex items-center gap-1.5 p-2 rounded-md bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 text-xs">
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {expiryStatus?.warning && <span>{expiryStatus.label === "Expired" ? "Cloud expired!" : "Cloud expiring soon"}</span>}
                      </div>
                    </div>
                  )}
                </CardContent>
      ...
      <AddStorageDeviceDialog
        open={drawerOpen}
        onOpenChange={setDrawerOpen}
        editDevice={editDevice}
        onSave={async (data) => {
          if (editDevice) await update(editDevice.id, data);
          else await add(data);
          setDrawerOpen(false);
        }}
      />
